...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    // Rotate mouse 180 degrees to match rotated fragment coordinates
    vec2 rotatedMouse = vec2(iResolution.x - iMouse.x, iResolution.y - iMouse.y);
    float mAct=step(1.0,length(rotatedMouse.xy)),nx=((rotatedMouse.x-C.x)*invW)*mAct;
    float ax = abs(nx);
...
void main(){
  ...
  vec2 rotated = vec2(iResolution.x - gl_FragCoord.x, iResolution.y - gl_FragCoord.y);
  mainImage(fc, rotated);
  gl_FragColor = fc;
}
`;
